...
@given('Open the main page of the Target')
def open_target_main_page(context):
    context.driver.get('https://www.target.com/')
@when("Search for Russian hat timecheck")
def target_button_pressed(context):
    context.driver.find_element(By.ID, 'search').send_keys('car')  # item search
    # sleep(2)
    context.driver.find_element(By.XPATH, "//button[@type='submit']").click()  # prees the loop
    sleep(5)
    context.driver.find_element(By.XPATH, "//picture[@class='styles__StyledFadeInPicture-sc-12vb1rw-0 kkdmHq']//@alt='Hot Wheels 20 Car Gift Pack'")  # press the button add to cart
    sleep(8)
